...
for i in range(4):  # 4 sets: [0-2], [3-5], [6-8], [9-11]
    ...
    pca_upsampled = TF.resize(
        pca_rgb,  # CHW format
        list(rgb.shape[:-1][::-1]),  # (height, width)
...
server.scene.add_point_cloud( name="/moge_aligned_dinopca", points=moge_aligned, colors=pca_upsampled_list[1].reshape(-1, 3)[mask_flat].astype(np.float32), point_size=0.001,)
