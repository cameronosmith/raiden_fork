...
class SO100AdhesiveConfig(ExoskeletonConfig):
    ...
    links = {
        ...
        "fixed_gripper": LinkConfig( mujoco_name="fixed_gripper", pybullet_name="Fixed_Jaw", robot_mesh_path=f"Fixed_Jaw.stl", exo_mesh_path=f"{BLENDER_STL_DIR}/fixed_gripper_exo.stl",
                                    aruco_offset_pos=np.array([36.73, -30.9, 0]), aruco_offset_rot=np.array([0, np.pi/2, 0]), aruco_board_name="fixed_gripper", board_length=0.045,),
                                    #aruco_offset_pos=np.array([35.73, -30.9, 0]), aruco_offset_rot=np.array([0, np.pi/2, 0]), aruco_board_name="fixed_gripper", board_length=0.045,),
    }
