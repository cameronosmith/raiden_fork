...
class FeetechMotorsBus:
    ...
    def connect(self):
        if self.is_connected:
            raise Exception( f"FeetechMotorsBus({self.port}) is already connected. Do not call `motors_bus.connect()` twice.")
        ...
        # Post-open: flush serial buffers and reset caches
        try:
            ...
            pass
        self.group_readers = {}
        self._port_sig = id(self.port_handler)
        # Allow to read and write
        self.is_connected = True

        # flush serial buffers (best effort)
        try:
            self.port_handler.ser.reset_input_buffer()
            self.port_handler.ser.reset_output_buffer()
        except Exception:
            pass
        # reset caches and set port signature
        self.group_readers = {}
        self._port_sig = id(self.port_handler)
    ...
    def read(self, data_name, motor_names: str | list[str] | None = None):
        ...
        if (group_key not in self.group_readers) or (self._port_sig != id(self.port_handler)):
